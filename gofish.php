...
function save_game(){
	global $GAME_STATE;
	$state_json = json_encode($GAME_STATE, JSON_PRETTY_PRINT);
	file_put_contents("game.json", $state_json);
}


function load_game(){
	global $GAME_STATE;
	$state_json = file_get_contents("game.json");
	$GAME_STATE = json_decode($state_json, true);
}
